...
namespace Game
{
    public class DisplayController : MonoBehaviour
    {
        ...
        private static string Reverse(string s)
        {
            ...
            Array.Reverse(charArray);           // Revers char array
            return new string(charArray);       // convert char array to string and return it
        }

        private string IntToString(int number)
        {
            string finishedNumber = number.ToString();
            while (finishedNumber.Length < 5) finishedNumber = "0" + finishedNumber;
